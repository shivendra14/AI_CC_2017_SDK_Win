function OnFinish(selProj, selObj) {
    try {
        ...
        SetPreCompileForExisting(selProj);
        AddFilesToNewProjectWithInfFile(selProj, strProjectName);
        AddConfig(selProj, strProjectName);

        // remove the xcode project and resource created
        selProj.Object.RemoveFile(selProj.Object.Files.Item("project.pbxproj"));
        selProj.Object.RemoveFile(selProj.Object.Files.Item( strProjectName + ".r" ));
...
function CreateCustomProject(strProjectName, strProjectPath) {
    try {
        ...
        var strProjTemplate = '';
        strProjTemplate = strProjTemplatePath + '\\IllustratorSDKdefault.vcxproj';
...
function AddConfig(proj, strProjectName) {
    try {
     ...
     //   config.IntermediateDirectory = '..\\output\\objects\\$(Platform)\\' + strProjectName + '\\$(Configuration)\\';
     //   config.OutputDirectory = '..\\output\\win\\$(Platform)\\$(Configuration)\\';
     ...
     //   CLTool.StructMemberAlignment = 3; // 4 Bytes (/Zp4)
        ...
        CLTool.UsePrecompiledHeader = 1; // Create Precompiled Header (/Yc)
     //   CLTool.PrecompiledHeaderThrough = 'IllustratorSDK.h';
        CLTool.PrecompiledHeaderFile = '$(IntDir)' + strProjectName + '.pch'

        // C/C++ > Output Files
     //   CLTool.AssemblerListingLocation = '$(IntDir)';
     //   CLTool.ObjectFile = '$(IntDir)';
     ...
     //   LinkTool.OutputFile = '$(OutDir)' + strProjectName + '.aip';
      //  LinkTool.SuppressStartupBanner = 1; // Yes (/NOLOGO)
      //  LinkTool.ModuleDefinitionFile = '..\\common\\win\\PluginMain.def';
    //    LinkTool.GenerateDebugInformation = 1; // Yes (/Debug)
    ...
    //    LinkTool.SubSystem = 2; // Windows (/SUBSYSTEM:WINDOWS)
     ...
     //   ResourceTool.Culture = 0x0409; // English (United States)
     //   ResourceTool.AdditionalIncludeDirectories = '.\\Source,..\\Source,..\\common\\win;..\\common\\includes;..\\..\\common\\win;..\\..\\common\\includes';
    ...
    /*    config = proj.Object.Configurations('Release|Win32');
      ...
      //  config.ConfigurationType = 2; // Dynamic Library (.dll)
        ...
        var LinkTool = config.Tools('VCLinkerTool');
        LinkTool.OutputFile = '$(OutDir)' + strProjectName + '.aip';
        LinkTool.SuppressStartupBanner = 1;
        LinkTool.ModuleDefinitionFile = '..\\common\\win\\PluginMain.def';
...
function GetTargetName(strName, strProjectName) {
    try {
        ...
        switch (strName) {
            case 'Source\\TemplatePlugin.cpp':
                strTarget = 'Source\\' + strProjectName + 'Plugin.cpp';
                break;
            case 'Source\\TemplatePlugin.h':
                strTarget = 'Source\\' + strProjectName + 'Plugin.h';
                break;
            case 'Source\\TemplateSuites.cpp':
                strTarget = 'Source\\' + strProjectName + 'Suites.cpp';
                break;
            case 'Source\\TemplateSuites.h':
                strTarget = 'Source\\' + strProjectName + 'Suites.h';
                break;
            case 'Source\\TemplateID.h':
                strTarget = 'Source\\' + strProjectName + 'ID.h';
                break;
            case 'Resources\\Win\\resource.rc':
                strTarget = 'Resources\\Win\\' + strProjectName + '.rc';
                break;
            case 'Resources\\Mac\\Template.r':
                strTarget = 'Resources\\Mac\\' + strProjectName + '.r';
                break;
            case 'Template.xcodeproj\\project.pbxproj':
                strTarget = strProjectName + '.xcodeproj\\project.pbxproj';
                break;
...
function SetFileProperties(projfile, strName) {
    if (strName == 'Source\\TemplatePlugin.cpp') {
    ...
    else if (strName == 'Source\\TemplatePlugin.h') {
        projfile.Object.FileType = eFileTypeCppHeader;
        var prj = projfile.Object.project;
        var folder = prj.Filters.Item('Source Files');
        projfile.Object.Move(folder);
    }
    else if (strName == 'Source\\TemplateSuites.cpp') {
        projfile.Object.FileType = eFileTypeCppCode;
        var prj = projfile.Object.project;
        var folder = prj.Filters.Item('Source Files');
        projfile.Object.Move(folder);
    }
    else if (strName == 'Source\\TemplateSuites.h') {
        projfile.Object.FileType = eFileTypeCppHeader;
        var prj = projfile.Object.project;
        var folder = prj.Filters.Item('Source Files');
        projfile.Object.Move(folder);
    }
    else if (strName == 'Source\\TemplateID.h') {
        projfile.Object.FileType = eFileTypeCppHeader;
        var prj = projfile.Object.project;
        var folder = prj.Filters.Item('Source Files');
        projfile.Object.Move(folder);
    }
    else if (strName == 'Resources\\Win\\resource.rc') {
        projfile.Object.FileType = eFileTypeRC;
        var prj = projfile.Object.project;
        var folder = prj.Filters.Item('Resource Files');
        projfile.Object.Move(folder);
    }
    else if (strName == 'Resources\\Win\\resource.h') {
        projfile.Object.FileType = eFileTypeCppHeader;
        var prj = projfile.Object.project;
...
function AddExistingFiles(strProjectPath) {
    ...
    itemOp.AddExistingItem(strProjectPath + '\\..\\common\\source\\Suites.cpp');
    itemOp.AddExistingItem(strProjectPath + '\\..\\common\\source\\Plugin.cpp');
    itemOp.AddExistingItem(strProjectPath + '\\..\\common\\source\\Main.cpp');
    itemOp.AddExistingItem(strProjectPath + '\\..\\common\\source\\AppContext.cpp');
    itemOp.AddExistingItem(strProjectPath + '\\..\\..\\illustratorapi\\illustrator\\IAIUnicodeString.cpp');
}

function SetPreCompileForExisting(selProj) {
    ChangePreCompileSettingsAll(selProj, "Suites.cpp")
    ChangePreCompileSettingsAll(selProj, "Plugin.cpp")
    ChangePreCompileSettingsAll(selProj, "Main.cpp")
    ChangePreCompileSettingsAll(selProj, "AppContext.cpp")
    ChangePreCompileSettingsAll(selProj, "IAIUnicodeString.cpp")
}

function ChangePreCompileSettingsAll(selProj, filestr) {
    ChangePreCompileSettings(selProj, filestr, 'Debug|Win32');
    ChangePreCompileSettings(selProj, filestr, 'Release|Win32');
    ChangePreCompileSettings(selProj, filestr, 'Debug|x64');
    ChangePreCompileSettings(selProj, filestr, 'Release|x64');
}

function ChangePreCompileSettings(selProj,filestr,env)
{
	var config = selProj.Object.Configurations(env);
	var files = selProj.Object.Files;
	var file = files.Item(filestr);
	var stdafxFileConfig = file.FileConfigurations(env);
	var CompilerTool = stdafxFileConfig.Tool;
