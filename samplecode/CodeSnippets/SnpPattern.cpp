//========================================================================================
//  
//  $File: //ai_stream/rel_21_0/devtech/sdk/public/samplecode/CodeSnippets/SnpPattern.cpp $
//
//  $Revision: #1 $
//
				...
				for (int swatchIndex = 0; swatchIndex < numSwatches; swatchIndex++){
					if(( swatchRef = sAISwatchGroup->GetNthSwatch(groupRef,swatchIndex)) != NULL){
						...
						if(color.kind == kPattern){
							...
							if(type == kPathArt){
								AIPathStyle style;
								result = sAIPathStyle->GetPathStyle(art,&style);
								aisdk::check_ai_error(result);
...
ASErr SnpPattern :: CreatePattern(void)
{
	...
	try {
		...
		// Set style with no fill and stroke.
		AIPathStyle style;
		result = sAIPathStyle->GetPathStyle(basePath,&style);
		aisdk::check_ai_error(result);
