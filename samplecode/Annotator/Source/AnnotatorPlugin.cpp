//========================================================================================
//  
//  $File: //ai_stream/rel_21_0/devtech/sdk/public/samplecode/Annotator/Source/AnnotatorPlugin.cpp $
//
//  $Revision: #1 $
//
...
ASErr AnnotatorPlugin::TrackToolCursor(AIToolMessage* message)
{
	...
	try {
		if (this->fAnnotator) {
			...
			aisdk::check_ai_error(result);
			if(sAIUser != NULL)
				result = sAIUser->SetCursor(kAnnotatorToolIconResourceID, fResourceManagerHandle);
            aisdk::check_ai_error(result);
		}
	}
	catch (ai::Error& ex) {
...
ASErr AnnotatorPlugin::AddTool(SPInterfaceMessage *message)
{
	...
	try {
		ASErr			error = kNoErr;
		AIAddToolData	data;
		data.title = "SDK Annotator Tool"; 
		data.tooltip = "SDK Annotator Tool";
