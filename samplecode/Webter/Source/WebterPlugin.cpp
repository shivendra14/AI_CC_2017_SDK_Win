//========================================================================================
//  
//  $File: //ai_stream/rel_21_0/devtech/sdk/public/samplecode/Webter/Source/WebterPlugin.cpp $
//
//  $Revision: #1 $
//
...
ASErr WebterPlugin::ShutdownPlugin( SPInterfaceMessage *message )
{
	if (fWebterPanelController)
	{
		...
		fWebterPanelController->SetDocumentDictionary(NULL);
...
ASErr WebterPlugin::Notify(AINotifierMessage *message)
{
	...
	if (message->notifier == fArtSelectionChangedNotifier)
	{
		...
		if (theArtStore)
		{
			...
			fWebterPanelController->SetArtDictionary(theDictionary, artName);
			sAIMdMemory->MdMemoryDisposeHandle((void**)theArtStore);
			sAIDictionary->Release(theDictionary);
		}
		else
		{
			fWebterPanelController->SetArtDictionary(NULL, ai::UnicodeString("No art selected"));
	...
	else if (message->notifier == fDocumentChangedNotifier)
	{
		...
		{
			// No document, so no artwork ==> clear artwork dictionary too
			fWebterPanelController->SetArtDictionary(NULL,
													 ai::UnicodeString("No art selected"));
		}
		
		fWebterPanelController->SetDocumentDictionary(theDictionary);
		sAIDictionary->Release(theDictionary);
	}
